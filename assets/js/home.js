function assignedClick(){
    ...
    fetch('https://deliverit-backend.vercel.app/roundsman/ordersName/'+cookieValue, options)
...
function completedClick(){
    ...
    .find((row)=>row.startsWith('idRoundsman='))
    ?.split('=')[1];
    fetch('https://deliverit-backend.vercel.app/roundsman/ordersCompleted/'+cookieValue, options)
    .then(response => response.json())
    .then(response => {
